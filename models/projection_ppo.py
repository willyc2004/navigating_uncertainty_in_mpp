...
class Projection_SinglePPO(RL4COLitModule):
    """
    ...
    including:
    - Generalized Advantage Estimation (GAE) (https://arxiv.org/abs/1506.02438) is not applicable
    since we are dealing with a single-step MDP.
    ...
    def shared_step(
        ...
        with torch.no_grad():
            ...
            out = self.policy(td.clone(), self.env, phase=phase, return_actions=True,
                              return_logits=True, return_feasibility=False,
                              projection_type=self.projection_layer, projection_kwargs=self.projection_kwargs)
        ...
        if phase == "train":
            ...
            td.set("logits", out["logits"])
            td.set("proj_mean_logits", out["proj_mean_logits"])
            ...
            for _ in range(self.ppo_cfg["ppo_epochs"]):  # PPO inner epoch, K
                for sub_td in dataloader:
                    ...
                    out = self.policy(  # note: remember to clone to avoid in-place replacements!
                        ...
                        return_feasibility=True,
                        return_logits=True,
                        decode_type="continuous_evaluate",
                        projection_type=self.projection_layer,
                        projection_kwargs=self.projection_kwargs,
                    )
                    # Outputs from the policy
                    ll, entropy, logits, proj_mean_logits = out["log_likelihood"], out["entropy"], out["logits"], \
                    out["proj_mean_logits"]
                    lhs_A, rhs = out["lhs_A"], out["rhs"]

                    # Compute the ratio of probabilities of new and old actions
                    ratio = torch.exp(ll.sum(dim=(-2, -1)) - sub_td["logprobs"].sum(dim=(-2, -1))).view(-1,1)  # [batch, 1]

                    # Compute the advantage
                    value_pred = self.critic(sub_td)  # [batch, 1]
                    adv = previous_reward - value_pred.detach()  # [batch, 1]
                    ...
                    feasibility_loss = F.mse_loss(self.lambda_violations * violation, torch.zeros_like(violation),
                                                  reduction="mean")
                    projection_loss = F.mse_loss(logits[..., 0], proj_mean_logits, reduction="mean")
