...
class Memory:
    def __init__(self, td, max_steps, action_shape, logprobs_shape, device="cuda"):
        batch_size = td.batch_size
        self.action_shape = action_shape
        self.tds_obs = []
        self.actions = torch.zeros((*batch_size, max_steps, action_shape), device=device)
        self.logprobs = torch.zeros((*batch_size, max_steps, logprobs_shape), device=device)
...
class Projection_PPO(RL4COLitModule):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.projection_kwargs = kwargs.pop("projection_kwargs", {})  # pop before passing to super
        ...
        if env.name == "mpp":
            ...
            self.select_obs_td = ["obs", "done", "timestep", "action_mask", "lhs_A", "rhs", "clip_min", "clip_max",
                                  ("state", "utilization"), "realized_demand"]
        else:
            self.lambda_violations = torch.ones(1, device='cuda', dtype=torch.float32)
            self.select_obs_td = None

        # Replay buffer
        self.rb = make_replay_buffer(buffer_size, self.mini_batch_size, buffer_storage_device)
    ...
    def _compute_losses_mpp(self, out, batch, td, lambda_values, tolerance=1e-4, alpha=0.5,):
        """Compute the PPO loss, value loss, entropy loss, feasibility loss, and projection loss."""
        # Extract from memory/out
