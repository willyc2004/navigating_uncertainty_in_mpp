...
from rl4co.rl4co.projection_n_step_ppo import check_for_nans
...
class MPPInitEmbedding(nn.Module):
    ...
    def forward(self, td: TensorDict):
        ...
        combined_input = torch.cat([#expected_demand, std_demand,
                                    *cargo_parameters.values(),
                                    ], dim=-1)
...
class MPPContextEmbedding(nn.Module):
    ...
    def __init__(self, action_dim, embed_dim, env, demand_aggregation="full",):
        ...
        self.env = env
        self.seq_size = self.env.T * self.env.K
        self.project_context = nn.Linear(20, embed_dim,) # 286, 217, 20, 142, embed_dim +
    ...
    def forward(self,
        ...
        context_embedding = torch.cat([state_embedding], dim=-1)
    ...
    def _state_embedding(self, td):
        ...
        state_embed = torch.cat([
            # current_demand, #observed_demand, #expected_demand, std_demand,
            residual_capacity, #residual_lc_capacity, origin_embed, destination_embed
        ], dim=-1)
        # print("state_embed", state_embed.mean())
        check_for_nans(state_embed, "state_embed")
        return state_embed
